...
<Steps>
  <Step title="Run the scaffolder">
    ...
    ```
    This creates a new directory with a complete help center project. `create-helpbase` is intended for greenfield projects.
...
If you already have a Next.js + shadcn project, use the one-command installer:

```bash
pnpm dlx helpbase init
```

This adds the full helpbase primitive (docs, MCP server, and sync workflow) to your repo in one shadcn drop.
